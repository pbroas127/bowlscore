...
const fixture = (id: string, brand: string, name: string, species: 'dog' | 'cat', form: CatalogProduct['form'], priceTier: 1 | 2 | 3, score: number, ingredients: string[], flags: Flag[], dm: [number, number, number]): CatalogProduct => ({
  id, brand, name, species, form, priceTier, lifeStage: 'all', image: null,
  label: { productName: name, brand, foodForm: form === 'treat' ? 'dry' : form, isTreat: form === 'treat', aafco: form === 'treat' ? 'not_found' : 'complete', ingredients },
...
export const SAMPLE_CATALOG: Catalog = {
  ...
  products: [
    ...
    fixture('sample-dog-wet', 'Sample pantry', 'Turkey and Pumpkin Stew', 'dog', 'wet', 3, 88, ['Turkey', 'Turkey broth', 'Turkey liver', 'Pumpkin', 'Carrots', 'Flaxseed'], [meatFirst('Turkey'), { severity: 'good', title: 'High protein', detail: 'About 42% protein once water is removed.' }], [42, 24, 18]),
    fixture('sample-dog-treat', 'Sample bakery', 'Single Ingredient Beef Liver Bites', 'dog', 'treat', 1, 84, ['Beef liver'], [meatFirst('Beef liver'), { severity: 'info', title: 'This is a treat', detail: 'Scored on ingredients only. Treats should stay under ten percent of daily calories.' }], [62, 12, 8]),
    fixture('sample-cat-dry', 'Sample feline', 'Salmon and Turkey Recipe', 'cat', 'dry', 2, 86, ['Deboned salmon', 'Turkey meal', 'Peas', 'Chicken fat (preserved with mixed tocopherols)', 'Taurine'], [meatFirst('Deboned salmon'), naturalPreservatives, { severity: 'good', title: 'Taurine included', detail: 'Cats cannot make enough taurine on their own, and this food adds it.' }], [40, 18, 27]),
  ],
}
