...
export async function GET(req: Request) {
  ...
  try {
    ...
    const res = await fetch(FDA_RECALLS_URL, { headers: { 'User-Agent': 'BowlScore/1.0 (pbroas127+bowlscore@gmail.com)', Accept: 'application/json' }, next: { revalidate: SIX_HOURS }, signal: AbortSignal.timeout(20_000) })
